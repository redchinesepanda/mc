...
class ToolPermalink
{
    ...
    public static function register_functions_admin()
    {
        ...
        {
            if ( MultisiteBlog::check_not_main_blog() && self::check_custom_permalink() )
            {
                $handler = new self();
    ...
    public static function get_post_uri( $post_id )
    {
        ...
        LegalDebug::die( [
            ...
            'gettype' => gettype( $post_id ),

            'permalink_manager_uris' => $permalink_manager_uris,

            'not_empty' => ( ! empty( $permalink_manager_uris[ $post_id ] ) ),
        ] );
        ...
        if ( $permalink_manager_uris )
        {
            ...
            if ( array_key_exists( $post_id, $permalink_manager_uris ) )
            {
                // LegalDebug::die( [
                //     'ToolPermalink' => 'get_post_uri-2',

                //     'permalink_manager_uri' => $permalink_manager_uris[ $post_id ],
                // ] );
